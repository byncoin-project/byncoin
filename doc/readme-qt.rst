Byncoin-qt: Qt4 GUI for Byncoin
===============================
...
Alternatively, install Qt Creator and open the `byncoin-qt.pro` file.

An executable named `byncoin-qt` will be built.
...
A warning for people using the *static binary* version of Byncoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Byncoin is linked against libdb5.3.
...
installed causes byncoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
