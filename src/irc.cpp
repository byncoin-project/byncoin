...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #byncoinTEST2\r");
            Send(hSocket, "WHO #byncoinTEST2\r");
        } else {
            // randomly join #byncoin00-#byncoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #byncoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #byncoin%02d\r", channel_number).c_str());
        }
